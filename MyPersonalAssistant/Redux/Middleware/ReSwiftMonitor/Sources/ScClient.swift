...
public class ScClient: Listener {
    ...
    private func sendHandShake() {
        let handshake = Model.getHandshakeObject(authToken: authToken, messageId: counter.incrementAndGet())
    ...
    public func subscribe(channelName: String) {
        ...
        socket.write(string: subscribeObject.toJSONString()!)
    }
                ...
                if let (data, rid, cid, eventName, error) = Parser.getMessageDetails(myMessage: messageObject) {
                    ...
                    case .isAuthenticated:
                        ...
                        if isAuthenticated != nil {
                            socketId = ClientUtils.getSocketId(message: messageObject)
                        }
                    ...
                    case .setToken:
                        ...
                        onSetAuthentication?(self, authToken)
                    case .ackReceive:

                        handleEmitAck(id: rid!, error: error as AnyObject, data: data as AnyObject)
                    case .event:
                        if hasEventAck(eventName: eventName!) {
                            handleOnAckListener(eventName: eventName!, data: data as AnyObject, ack: ack(cid: cid!))
                        } else {
