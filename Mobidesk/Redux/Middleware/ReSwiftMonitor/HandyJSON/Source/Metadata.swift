...
struct _class_rw_t {
    ...
    func class_ro_t() -> UnsafePointer<_class_ro_t>? {
        return UnsafePointer<_class_ro_t>(bitPattern: ro)
    }
}
        ...
        func _propertyDescriptionsAndStartPoint() -> ([Property.Description], Int32?)? {
            ...
            if let fieldOffsets = self.fieldOffsets {
                ...
                for i in 0 ..< numberOfFields {
                    if let name = reflectionFieldDescriptor?.fieldRecords[i].fieldName,
                        let cMangledTypeName = reflectionFieldDescriptor?.fieldRecords[i].mangledTypeName,
    ...
    struct _Class {
        ...
        func class_rw_t() -> UnsafePointer<_class_rw_t>? {
            if MemoryLayout<Int>.size == MemoryLayout<Int64>.size {
                ...
                let databits_t: UInt64 = UInt64(rodataPointer)
